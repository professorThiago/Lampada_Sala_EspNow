...
void iniciarEspNow();
void tratarWiFi();
bool conectarWifi(uint32_t timeoutMs, bool comTrava);
...
void setup()
{
  ...
  bool ok = conectarWifi(MS_BOOT_CONNECT, true);    // 1a tentativa: como configurado
  if (!ok && cfg.usarBssid)
  {
    // A trava (BSSID/canal fixo) pode apontar para um AP inexistente/invalido.
    // Fallback de recuperacao: tenta a MESMA rede SEM travar, para nao perder o
    // acesso a web e permitir corrigir/desligar o BSSID pela interface.
    Serial.println("[WiFi] Falhou com BSSID/canal fixos -> tentando SOLTO (fallback de recuperacao).");
    ok = conectarWifi(MS_BOOT_CONNECT, false);
  }
...
// Tenta conectar dentro de timeoutMs. comTrava=false ignora BSSID/canal fixos
// (fallback de recuperacao quando a trava aponta para um AP invalido).
bool conectarWifi(uint32_t timeoutMs, bool comTrava)
{
  const uint8_t *bssid = (comTrava && cfg.usarBssid) ? cfg.bssid : nullptr;
  uint8_t canal        = comTrava ? cfg.canal : 0;

  WiFi.disconnect();
  WiFi.begin(cfg.wifiSsid, cfg.wifiSenha, canal, bssid);
  Serial.printf("[WiFi] Conectando a \"%s\" (canal:%d, bssid:%s)",
                cfg.wifiSsid, canal, bssid ? bssidParaStr(cfg.bssid).c_str() : "auto");
